...
def test_ct_relationship_new_internal() -> None:
    ...
    expected_xml = (
        '<Relationship xmlns="http://schemas.openxmlformats.org/package/2006/relationships" '
        'Id="rId9" Type="ReLtYpE" Target="foo/bar.xml"/>'
    )
    assert xml_string_eq_ignores_attribute_order(rel.xml, expected_xml)


def test_ct_relationship_new_external() -> None:
    ...
    expected_xml = (
        '<Relationship xmlns="http://schemas.openxmlformats.org/package/2006/relationships" '
        'Id="rId9" Type="ReLtYpE" Target="http://some/link" TargetMode="External"/>'
    )
    assert xml_string_eq_ignores_attribute_order(rel.xml, expected_xml)


def test_ct_relationships_new() -> None:
    rels = CT_Relationships.new()

    assert rels.tag.endswith("Relationships")
    assert rels.relationship_lst == []


def test_ct_relationships_add_rel() -> None:
    rels = CT_Relationships.new()
    ...
    assert xml_string_eq_ignores_attribute_order(rels.xml, expected_xml)


def test_ct_relationships_xml_file_bytes() -> None:
    rels = CT_Relationships.new()

    assert b"Relationships" in rels.xml_file_bytes


def test_ct_types_new() -> None:
    types = CT_Types.new()

    assert types.tag.endswith("Types")
    assert types.default_lst == []
    assert types.override_lst == []


def test_ct_types_add_default_override() -> None:
    types = CT_Types.new()
    ...
    assert xml_string_eq_ignores_attribute_order(types.xml, expected_xml)


def test_serialize_part_xml() -> None:
    part_elm = parse_xml('<f:foo xmlns:f="http://foo"><f:bar>foobar</f:bar></f:foo>')
    xml_bytes = serialize_part_xml(part_elm)

    assert b"standalone='yes'" in xml_bytes
    assert b"<f:bar>foobar</f:bar>" in xml_bytes
